                ...
                <div class="xl:col-span-2 space-y-6">
                    ...
                    <div>
                        ...
                        <textarea class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm @error('key_features') border-red-300 @enderror" 
                                  ...
                                  placeholder="Enter key features separated by commas">{{ old('key_features', $product->key_features) }}</textarea>
                        @error('key_features')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        ...
                        <div class="space-y-4">
                                    ...
                                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                        @foreach($product->images->sortBy('sort_order') as $image)
                                            <div class="relative group">
                                                <img src="{{ $image->image_url }}" alt="Product image" class="w-full h-24 object-cover rounded-lg border border-gray-200">
                            ...
                            <div>
                                <label for="other_images" class="block text-sm font-medium text-gray-700">Add New Other Images</label>
                                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-gray-400 transition-colors duration-200">
                                    <div class="space-y-1 text-center">
                                        <svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                                            <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8m-12 4h.02" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm @error('meta_keywords') border-red-300 @enderror" 
                                       id="meta_keywords" name="meta_keywords" value="{{ old('meta_keywords', $product->meta_keywords) }}" placeholder="keyword1, keyword2, keyword3">
                                @error('meta_keywords')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
...
<script type="text/javascript">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Load existing attribute blocks for editing
        loadExistingAttributeBlocks();
    });

    // Load existing attribute blocks for editing
    window.loadExistingAttributeBlocks = function() {
        const existingAttributes = @json($product->attributeValues ?? []);
        
        if (existingAttributes.length > 0) {
            existingAttributes.forEach((attrValue, index) => {
                attributeBlockCounter++;
                const blockId = `attribute-block-${attributeBlockCounter}`;
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <select name="attribute_blocks[${attributeBlockCounter}][attribute_id]" 
                                        ...
                                        <option value="">Select Attribute</option>
                                        ${availableData.attributes.map(attr => 
                                            `<option value="${attr.id}" ${attr.id == attrValue.attribute_id ? 'selected' : ''}>${attr.name}</option>`
                                        ).join('')}
                                        <option value="add_new" class="text-primary-600 font-medium">+ Add New Attribute</option>
                                    </select>
                                </div>
                            ...
                            <div class="attribute-value-container">
                                ...
                                <div class="relative">
                                    <select name="attribute_blocks[${attributeBlockCounter}][attribute_value_id]" 
                                            ...
                                            onchange="onAttributeValueChange(${attributeBlockCounter})">
                                        <option value="">Select Value</option>
                                        ${availableData.attributeValues.filter(val => val.attribute_id == attrValue.attribute_id).map(value => 
                                            `<option value="${value.id}" ${value.id == attrValue.id ? 'selected' : ''}>${value.value}</option>`
                                        ).join('')}
                                        <option value="add_new" class="text-primary-600 font-medium">+ Add New Value</option>
                                    </select>
                                </div>
