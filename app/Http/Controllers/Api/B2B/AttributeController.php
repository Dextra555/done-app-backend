...
class AttributeController extends Controller
{
    ...
    /**
     * Get attributes with usage statistics (B2B specific)
     */
    public function getWithUsageStats(): JsonResponse
    {
        try {
            $attributes = ProductAttribute::with(['subAttributes', 'values.subAttribute'])
                ->get()
            ...
            return response()->json([
                'status' => false,
                'message' => 'Failed to retrieve attributes with usage statistics',
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    /**
     * Get popular attributes (most used in variants)
     */
    public function getPopular(): JsonResponse
    {
        try {
            $popularAttributes = ProductAttribute::with(['subAttributes', 'values.subAttribute'])
                ->get()
                ->map(function ($attribute) {
                    $usageCount = $attribute->values()
                        ->whereHas('variants')
            ...
            return response()->json([
                ...
                'data' => $popularAttributes
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => false,
                'message' => 'Failed to retrieve popular attributes',
                'error' => $e->getMessage()
            ], 500);
        }
    }
